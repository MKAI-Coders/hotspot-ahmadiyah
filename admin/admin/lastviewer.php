...
<div class="content">
  ...
  <?php
   ...
   // $sql = "SELECT * FROM view_user WHERE identity='$identity' ORDER BY id DESC";
	$sql = "SELECT * FROM view_user ORDER BY id DESC";
   
    $retval = mysql_query($sql);
	$totaldata = mysql_num_rows($retval);
	
	$sql = "SELECT * FROM view_user WHERE nama=''";
	// $sql = "SELECT * FROM view_user WHERE identity='$identity' AND nama=''";
	$retval = mysql_query($sql);
	$unregistereduser = mysql_num_rows($retval);
	
	$sql = "SELECT * FROM view_user ORDER BY id DESC LIMIT 500";
	...
	if ($totaldata > 0)
	{
		 ...
		 echo "<p><h3>Data 500 Pengunjung Terakhir</p></h3>";
		 
		 echo "Viewers : ".$totaldata." times</br>";
		 echo "Registered/Unregistered Viewers: ".($totaldata-$unregistereduser)."/".$unregistereduser." times";
		 
		 echo "</br></br><table><tr><th>Tanggal</th><th>Nama</th><th>MAC Address</th><th>Perangkat</th><th>Sistem Operasi</th><th>Browser</th><th>Client</th></tr>";
		 $i = 1;
